...
var 
	...
	C_TWEET_FADEUP_DURATION_PERCENT = 20,
	C_TWEET_FADELIVE_DURATION_PERCENT = 70,
...
d3.cloudshapes.fhtweetmanager = function module() {
	var mainSvg;
	var width,height;
	...
	var dispatch = d3.dispatch('removeTweetEvent', 'mouseTabletClick', 'tweetBorn');
...
//			tweet_data.y = 100;
			...
			var t_new_user_details = t_new_tweet.append("text")
				...
				.style({opacity: 1}) // was zero
			t_new_user_details.append("a")
				.attr("xlink:href", function(d)	{
					var t_user_details = d['user_details']
					return t_user_details['url'];
				})
				.attr("class", "text_user_details_link")
				.attr("target","_blank") 
				.text(function(d)	{
					var t_user_details = d['user_details']
					return t_user_details['text'];
				})
				.append("title")
					.text(function(d)	{
					var t_user_details = d['user_details']
			...
			var t_sentiment_colour = sentiment_colours[t_sentiment_colour_index];
			...
			for (var i=0; i<t_text_lines.length;i++)	{
				...
				var t_text_items = t_text_line.text;
				...
				for (var j=0; j<t_text_items.length;j++)	{
					var t_text_item_type = t_text_items[j].type;
					var t_text_item_text = t_text_items[j].text;


					switch(t_text_item_type)	{
						case "tspan":
							tweet_text.append("tspan")
								.attr("class", "tweet_text_tspan")
								.style({opacity: 1, fill:t_sentiment_colour}) // was zero
								.text(t_text_item_text);
						break;

						case "keyword_higlight":
							tweet_text.append("tspan")
								.attr("class", "tweet_keyword_higlight")
								.style({opacity: 1})
								.text(t_text_item_text);
						break;

						case "link":
							var t_text_item_url = t_text_items[j].url;
							tweet_text.append("a")
								.attr("xlink:href", t_text_item_url)
								.attr("class", "tweet_text_link")
								.attr("target","_blank") 
								.style({opacity: 1}) // was zero
								.text(t_text_item_text)
								.append("title")
			...
			datetime_text.append("a")
				.attr("xlink:href", tweet_data["tweet_link_details"]['url'])
				.attr("class", "datetime_details_link")
				.attr("target","_blank") 
				.text("(tweeted: " + tweet_data['formatted_date_created'] + ")")
				.append("title")
					.text("Link to live tweet");
	...
	exports.tickFn = function(t_tweetdata)	{
		...
		if (t_tweetdata['status'] == C_TWEET_LIVE)	{
			...
			if (t_tweetdata[C_TWEET_BORN_TIME] == undefined)	{
				dispatch.tweetBorn(t_tweetdata);
